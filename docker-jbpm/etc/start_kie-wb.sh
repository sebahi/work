...
./update-jpa-config.sh $JBOSS_HOME/standalone/deployments/jbpm-console.war/WEB-INF/classes/META-INF/persistence.xml
...
./standalone.sh -b $JBOSS_BIND_ADDRESS -bmanagement $JBOSS_BIND_ADDRESS -Dactivemq.artemis.client.global.thread.pool.max.size=30 -Djboss.kie.connection_url="$KIE_CONNECTION_URL" -Djboss.kie.driver="$KIE_CONNECTION_DRIVER" -Djboss.kie.username="$KIE_CONNECTION_USER" -Djboss.kie.password="$KIE_CONNECTION_PASSWORD" -Dorg.kie.demo=false -Dorg.kie.example=false --server-config=standalone-full-kie-wb.xml
exit $?
